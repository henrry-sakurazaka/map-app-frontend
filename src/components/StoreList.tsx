...
const StoreList: React.FC = () => {
  ...
  const fetchWebsiteFromDuckDuckGo = async (
    ...
    try {
      const res = await fetch(
        `https://api.duckduckgo.com/?q=${encodeURIComponent(storeName)}&format=json`,
      );
      const data = await res.json();
      if (data?.AbstractURL) return data.AbstractURL;
      if (data?.RelatedTopics?.[0]?.FirstURL)
        return data.RelatedTopics[0].FirstURL;
      return null;
    } catch (err) {
  ...
  const handleCopyMapImage = async () => {
    ...
    try {
      ...
      console.error('html2canvas で失敗:', _err);
      alert('コピーに失敗しました');
    }
  };
  ...
  return (
    <div className="w-full sm:w-[400px] lg:w-1/3 md:w-[650px] bg-gray-100 p-4 rounded-xl shadow-lg h-[60vh] lg:h-[70vh] overflow-y-auto z-10">
      {selectedStore ? (
        <div>
